...
# tính các đường cao trong tam giác
ha = float(p*(p-a)*(p-b)*(p-c))/a
hb = float(p*(p-a)*(p-b)*(p-c))/b
hc = float(p*(p-a)*(p-b)*(p-c))/c
print('đường cao các cạnh tam giác ABC là: ',ha,hb,hc)